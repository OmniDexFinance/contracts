...
import "omnidex/nft-marketplace/interfaces/IWETH.sol";
import "omnidex/nft-marketplace/interfaces/ICollectionWhitelistChecker.sol";
...
contract ERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    * @param _WTLOSAddress: WTLOS address
   ...
   constructor(
       ...
       require(_WTLOSAddress != address(0), "Operations: WTLOS address cannot be zero");
    ...
    */
   function buyTokenUsingTLOS(address _collection, uint256 _tokenId) external payable nonReentrant {
       // Wrap TLOS
       IWETH(WTLOS).deposit{value: msg.value}();
    ...
    */
   function buyTokenUsingWTLOS(
       address _collection,
       uint256 _tokenId,
       uint256 _price
   ) external nonReentrant {
       IERC20(WTLOS).safeTransferFrom(address(msg.sender), address(this), _price);
   ...
   function claimPendingRevenue() external nonReentrant {
       ...
       IERC20(WTLOS).safeTransfer(address(msg.sender), revenueToClaim);
   ...
   function recoverFungibleTokens(address _token) external onlyOwner {
       require(_token != WTLOS, "Operations: Cannot recover WTLOS");
    ...
    * @param _withTLOS: whether the token is bought with TLOS (true) or WTLOS (false)
    */
   function _buyToken(
       address _collection,
       uint256 _tokenId,
       ...
       // Transfer WTLOS
       IERC20(WTLOS).safeTransfer(askOrder.seller, netPrice);
       ...
       // Emit event
       emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, _withTLOS);
   }
